...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		...
		try {
			xhr.open(request.aI, request.v, true);
		} catch (e) {
...
var $elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {V: fragment, Y: host, ab: path, ad: port_, ag: protocol, ah: query};
	});
...
var $elm$http$Http$request = function (r) {
	return $elm$http$Http$command(
		$elm$http$Http$Request(
			{ax: false, az: r.az, aD: r.aD, W: r.W, aI: r.aI, aT: r.aT, ar: r.ar, v: r.v}));
};
var $elm$http$Http$get = function (r) {
	return $elm$http$Http$request(
		{az: $elm$http$Http$emptyBody, aD: r.aD, W: _List_Nil, aI: 'GET', aT: $elm$core$Maybe$Nothing, ar: $elm$core$Maybe$Nothing, v: r.v});
};
var $elm$json$Json$Decode$list = _Json_decodeList;
var $author$project$PhotoGroove$Photo = F3(
	function (url, size, title) {
		return {an: size, aq: title, v: url};
	});
...
var $author$project$PhotoGroove$initialCmd = $elm$http$Http$get(
	{
		...
		v: 'https://elm-in-action.com/photos/list.json'
	});
var $author$project$PhotoGroove$Loading = {$: 0};
var $author$project$PhotoGroove$Medium = 1;
var $author$project$PhotoGroove$initialModel = {B: 1, H: 5, I: 5, J: 5, j: $author$project$PhotoGroove$Loading};
			...
			case 1:
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							j: A2($author$project$PhotoGroove$selectUrl, photo.v, model.j)
						}),
			...
			case 0:
				...
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{
							j: A2($author$project$PhotoGroove$selectUrl, url, model.j)
						}),
					$elm$core$Platform$Cmd$none);
			case 2:
				var size = msg.a;
				...
				if (!msg.a.$) {
					...
					if (!photos.b) {
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									j: $author$project$PhotoGroove$Errored('0 photos found')
								}),
							$elm$core$Platform$Cmd$none);
					} else {
						var first = photos.a;
						return _Utils_Tuple2(
							_Utils_update(
								model,
								{
									j: A2($author$project$PhotoGroove$Loaded, photos, first.v)
								}),
							$elm$core$Platform$Cmd$none);
					}
				} else {
					return _Utils_Tuple2(
						_Utils_update(
							model,
							{
								j: $author$project$PhotoGroove$Errored('Server Error!')
							}),
						$elm$core$Platform$Cmd$none);
				}
			case 5:
...
var $author$project$PhotoGroove$rangerSlider = F2(
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					A2(
					$author$project$PhotoGroove$rangerSlider,
					_List_fromArray(
						[
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_Utils_ap($author$project$PhotoGroove$urlPrefix, thumb.v)),
					$elm$html$Html$Attributes$title(
					thumb.aq + (' [' + ($elm$core$String$fromInt(thumb.an) + ' KB]'))),
					$elm$html$Html$Attributes$classList(
					_List_fromArray(
						[
							_Utils_Tuple2(
							'selected',
							_Utils_eq(selectedUrl, thumb.v))
						])),
					$elm$html$Html$Events$onClick(
					$author$project$PhotoGroove$ClickedPhoto(thumb.v))
				]),
			_List_Nil);
	});
	...
	function (photos, selectedUrl, model) {
		return _List_fromArray(
			[
				...
				_List_fromArray(
					[
						A3($author$project$PhotoGroove$viewFilter, $author$project$PhotoGroove$SlidHue, 'Hue', model.H),
						A3($author$project$PhotoGroove$viewFilter, $author$project$PhotoGroove$SlidRipple, 'Ripple', model.J),
						A3($author$project$PhotoGroove$viewFilter, $author$project$PhotoGroove$SlidNoise, 'Noise', model.I)
					])),
				A2(
				$elm$html$Html$h3,
				_List_Nil,
				...
				A2(
					$elm$core$List$map,
					$author$project$PhotoGroove$viewSizeChooser(model.B),
					_List_fromArray(
						[0, 1, 2]))),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
					[
						...
						$author$project$PhotoGroove$sizeToString(model.B))
					]),
				A2(
					$elm$core$List$map,
...
var $author$project$PhotoGroove$main = $elm$browser$Browser$element(
	{
		...
		aU: $author$project$PhotoGroove$update,
		aV: $author$project$PhotoGroove$view
	});
